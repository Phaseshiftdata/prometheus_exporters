...
func TestMainHelp(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"relay_exporter", "--help"}
		main()
		return
	}
	...
	if !strings.Contains(string(out), "relay_exporter") {
		t.Errorf("expected 'relay_exporter' in --help output, got:\n%s", out)
	}
}
...
func TestMainVersion(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"relay_exporter", "--version"}
		main()
		return
	}
	...
	if !strings.Contains(string(out), "relay_exporter") {
		t.Errorf("expected 'relay_exporter' in --version output, got:\n%s", out)
	}
}
...
func TestMainInvalidFlag(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"relay_exporter", "--bogus-flag-that-does-not-exist"}
		main()
		return
	}
