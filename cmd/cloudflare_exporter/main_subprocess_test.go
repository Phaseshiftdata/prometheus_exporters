...
func TestMainHelp(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"cloudflare_exporter", "--help"}
		main()
		return
	}
	...
	if !strings.Contains(string(out), "cloudflare_exporter") {
		t.Errorf("expected 'cloudflare_exporter' in --help output, got:\n%s", out)
	}
}
...
func TestMainVersion(t *testing.T) {
	...
	if !strings.Contains(string(out), "cloudflare_exporter") {
		t.Errorf("expected 'cloudflare_exporter' in --version output, got:\n%s", out)
	}
}
...
func TestMainInvalidFlag(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"cloudflare_exporter", "--bogus-flag-that-does-not-exist"}
		main()
		return
	}
