...
func TestMainHelp(t *testing.T) {
	...
	if !strings.Contains(string(out), "libvirt_exporter") {
		t.Errorf("expected 'libvirt_exporter' in --help output, got:\n%s", out)
	}
}
...
func TestMainVersion(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"libvirt_exporter", "--version"}
		main()
		return
	}
	...
	if !strings.Contains(string(out), "libvirt_exporter") {
		t.Errorf("expected 'libvirt_exporter' in --version output, got:\n%s", out)
	}
}
...
func TestMainInvalidFlag(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"libvirt_exporter", "--bogus-flag-that-does-not-exist"}
		main()
		return
	}
