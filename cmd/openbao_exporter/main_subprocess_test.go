...
func TestMainHelp(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"openbao_exporter", "--help"}
		main()
		return
	}
	...
	if !strings.Contains(string(out), "openbao_exporter") {
		t.Errorf("expected 'openbao_exporter' in --help output, got:\n%s", out)
	}
}
...
func TestMainVersion(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"openbao_exporter", "--version"}
		main()
		return
	}
	...
	if !strings.Contains(string(out), "openbao_exporter") {
		t.Errorf("expected 'openbao_exporter' in --version output, got:\n%s", out)
	}
}
...
func TestMainInvalidFlag(t *testing.T) {
	if os.Getenv("BE_MAIN") == "1" {
		os.Args = []string{"openbao_exporter", "--bogus-flag-that-does-not-exist"}
		main()
		return
	}
