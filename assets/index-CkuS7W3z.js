...
spec:
  ...
  libvirt-exporter:
    ...
    </div>`}function o(){let e=[[`--listen-address`,`127.0.0.1:9100`,`Address and port the HTTP server listens on.`],[`--allowed-source`,`<em>(required)</em>`,`Single IP address allowed to send scrape requests. The relay refuses to start if omitted.`],[`--tls-cert-file`,`<em>(optional)</em>`,`TLS certificate for the relay listener. When provided with <code>--tls-key-file</code>, the relay serves HTTPS.`],[`--tls-key-file`,`<em>(optional)</em>`,`TLS private key for the relay listener. When provided with <code>--tls-cert-file</code>, the relay serves HTTPS.`],[`--ca-cert`,`<em>(optional)</em>`,`CA certificate bundle for verifying target TLS certificates.`],[`--tls-skip-verify`,`false`,`Skip TLS certificate verification when connecting to targets.`],[`--proxy-timeout`,`10s`,`Timeout for proxy requests to targets.`],[`--concurrent-requests`,`100`,`Maximum number of concurrent proxy requests.`],[`--log-level`,`info`,`Log verbosity. One of <code>debug</code>, <code>info</code>, <code>warn</code>, <code>error</code>.`]].map(([e,t,n])=>`<tr><td><code>${e}</code></td><td><code>${t}</code></td><td>${n}</td></tr>`).join(``),t=[[`Relay functioning, target responded (any status)`,`<strong>200</strong> (target status reported in <code>relay_target_http_status</code>)`],[`Relay functioning, target timed out or unreachable`,`<strong>200</strong> (with <code>relay_target_response 0</code>, <code>relay_target_http_status 0</code>)`],[`Source IP not allowed`,`<strong>403</strong> Forbidden`],[`Missing or invalid query parameters`,`<strong>400</strong> Bad Request`],[`Concurrent request limit exceeded`,`<strong>429</strong> Too Many Requests`]].map(([e,t])=>`<tr><td>${e}</td><td>${t}</td></tr>`).join(``);return`
    <div class="section">
...
docker run -d --rm \\
  ...
  --allowed-source=203.0.113.10</code></pre>
    ...
    <div class="section">
      ...
      ${(e=>`<table><thead><tr><th>Metric</th><th>Type</th><th>Labels</th><th>Description</th></tr></thead><tbody>${e.map(([e,t,n,r])=>`<tr><td><code>${e}</code></td><td>${t}</td><td><code>${n}</code></td><td>${r}</td></tr>`).join(``)}</tbody></table>`)([[`relay_response`,`gauge`,`<em>(none)</em>`,`Whether the relay itself is functioning (always 1 when the relay returns HTTP 200).`],[`relay_target_response`,`gauge`,`<em>(none)</em>`,`Whether the target responded successfully (1 = success, 0 = timeout or unreachable).`],[`relay_target_http_status`,`gauge`,`<em>(none)</em>`,`HTTP status code returned by the target (0 when the target is unreachable).`]])}
...
spec:
  ...
  relay_exporter:
    ...
    </div>`}var s=`0.1.0`,c=Object.create(null);c[`/`]=e,c[`/network-exporter`]=t,c[`/ipsec-exporter`]=n,c[`/cloudflare-exporter`]=r,c[`/github-exporter`]=i,c[`/libvirt-exporter`]=a,c[`/relay-exporter`]=o;function l(){let e=window.location.hash.replace(/^#\/?/,`/`);return e===``?`/`:e}function u(e){return`<nav class="nav">
    <a href="#/" class="nav-brand">prometheus_exporters</a>
    <div class="nav-links">${[{to:`/`,label:`Home`,exact:!0},{to:`/network-exporter`,label:`Network`},{to:`/ipsec-exporter`,label:`IPsec`},{to:`/cloudflare-exporter`,label:`Cloudflare`},{to:`/github-exporter`,label:`GitHub`},{to:`/libvirt-exporter`,label:`Libvirt`},{to:`/relay-exporter`,label:`Relay`}].map(t=>{let n=t.exact?e===t.to:e.startsWith(t.to);return`<a href="#${t.to}" class="${n?`active`:``}">${t.label}</a>`}).join(``)}</div>
  </nav>`}function d(){return`<footer class="footer" role="contentinfo">
  ...
  </footer>`}function f(){let e=l(),t=document.getElementById(`root`);if(!t)return;let n=e in c?c[e]:c[`/`];t.innerHTML=`
    ${u(e)}
    <main class="main">${n()}</main>
    ${d()}
  `,p(e)}function p(e){let t=Object.create(null);t[`/`]=`Prometheus Exporters — Network, IPsec & Cloudflare`,t[`/network-exporter`]=`Network Exporter — Prometheus Exporters`,t[`/ipsec-exporter`]=`IPsec Exporter — Prometheus Exporters`,t[`/cloudflare-exporter`]=`Cloudflare Exporter — Prometheus Exporters`,t[`/github-exporter`]=`GitHub Exporter — Prometheus Exporters`,t[`/libvirt-exporter`]=`Libvirt Exporter — Prometheus Exporters`,t[`/relay-exporter`]=`Relay Exporter — Prometheus Exporters`,document.title=e in t?t[e]:t[`/`];let n=document.querySelector(`link[rel="canonical"]`);n||(n=document.createElement(`link`),n.rel=`canonical`,document.head.appendChild(n)),n.href=`https://prometheus_exporters.phaseshiftdata.com/${e===`/`?``:`#`+e}`}f(),window.addEventListener(`hashchange`,f);