...
spec:
  ...
  relay_exporter:
    ...
    </div>`}var c=`0.1.0`,l=Object.create(null);l[`/`]=e,l[`/network-exporter`]=t,l[`/ipsec-exporter`]=n,l[`/cloudflare-exporter`]=r,l[`/github-exporter`]=i,l[`/libvirt-exporter`]=a,l[`/openbao-exporter`]=o,l[`/relay-exporter`]=s;function u(){let e=window.location.hash.replace(/^#\/?/,`/`);return e===``?`/`:e}function d(e){return`<nav class="nav">
    <a href="#/" class="nav-brand">prometheus_exporters</a>
    <div class="nav-links">${[{to:`/`,label:`Home`,exact:!0},{to:`/network-exporter`,label:`Network`},{to:`/ipsec-exporter`,label:`IPsec`},{to:`/cloudflare-exporter`,label:`Cloudflare`},{to:`/github-exporter`,label:`GitHub`},{to:`/libvirt-exporter`,label:`Libvirt`},{to:`/openbao-exporter`,label:`OpenBao`},{to:`/relay-exporter`,label:`Relay`}].map(t=>{let n=t.exact?e===t.to:e.startsWith(t.to);return`<a href="#${t.to}" class="${n?`active`:``}">${t.label}</a>`}).join(``)}</div>
  </nav>`}function f(){return`<footer class="footer" role="contentinfo">
  ...
  </footer>`}function p(){let e=u(),t=document.getElementById(`root`);if(!t)return;let n=e in l?l[e]:l[`/`];t.innerHTML=`
    ${d(e)}
    <main class="main">${n()}</main>
    ${f()}
  `,m(e)}function m(e){let t=Object.create(null);t[`/`]=`Prometheus Exporters — Network, IPsec & Cloudflare`,t[`/network-exporter`]=`Network Exporter — Prometheus Exporters`,t[`/ipsec-exporter`]=`IPsec Exporter — Prometheus Exporters`,t[`/cloudflare-exporter`]=`Cloudflare Exporter — Prometheus Exporters`,t[`/github-exporter`]=`GitHub Exporter — Prometheus Exporters`,t[`/libvirt-exporter`]=`Libvirt Exporter — Prometheus Exporters`,t[`/openbao-exporter`]=`OpenBao Exporter — Prometheus Exporters`,t[`/relay-exporter`]=`Relay Exporter — Prometheus Exporters`,document.title=e in t?t[e]:t[`/`];let n=document.querySelector(`link[rel="canonical"]`);n||(n=document.createElement(`link`),n.rel=`canonical`,document.head.appendChild(n)),n.href=`https://prometheus_exporters.phaseshiftdata.com/${e===`/`?``:`#`+e}`}p(),window.addEventListener(`hashchange`,p);